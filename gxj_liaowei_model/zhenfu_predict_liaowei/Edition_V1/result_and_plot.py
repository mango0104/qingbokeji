...
def result_and_plot(real_liaowei_data, start_time, time_length, time_span, liaowei_list, parameter, prediction):
    ...
    # 绘制第一个图形: 最优振幅下的预测料位与实际料位对比
    liaowei_comparison_plot(real_liaowei_data, start_time, time_length, time_span, liaowei_list, axes[0])

    # 绘制第二个图形: 真实振幅下的预测料位与实际料位对比
    jinjiao_time_data = tjjh_time_mysql()
    liaowei_list_real = liaowei_predict_byReal(real_liaowei_data, jinjiao_time_data, start_time, time_length, time_span, prediction.slope, prediction.intercept)
    liaowei_comparison_plot(real_liaowei_data, start_time, time_length, time_span, liaowei_list_real, axes[1])
