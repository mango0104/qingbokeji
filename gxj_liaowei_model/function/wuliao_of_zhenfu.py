...
def plot_scatter_and_fit():
    ...
    try:
        ...
        # 筛选数据
        filtered_df = history_df[(history_df['zhenfu'] >= 23) & (history_df['zhenfu'] <= 34) &
                                 (history_df['hight_change'] >= 2.5) & (history_df['hight_change'] <= 4.3)]

        # 准备数据用于线性回归
        X = filtered_df['zhenfu'].values.reshape(-1, 1)
        y = filtered_df['hight_change'].values
        ...
        # 绘制散点图和拟合直线
        plt.scatter(filtered_df['zhenfu'], filtered_df['hight_change'], label='Data Points')
